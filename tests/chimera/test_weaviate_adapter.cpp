...
/**
 * @file test_weaviate_adapter.cpp
 * @brief Unit tests for WeaviateAdapter (CHIMERA Suite)
 *
 * @details Tests cover connection management, vector search (primary
 *          capability), document CRUD, capability reporting, and
 *          NOT_IMPLEMENTED paths for unsupported operations (relational,
 *          graph, transactions).
 *
 * All tests run without a live Weaviate server – the adapter operates in
...
class WeaviateConnectionTest : public ::testing::Test {
protected:
    WeaviateAdapter adapter;
};

TEST_F(WeaviateConnectionTest, InitiallyDisconnected) {
    EXPECT_FALSE(adapter.is_connected());
}

TEST_F(WeaviateConnectionTest, ConnectWithHttpUri) {
    auto result = adapter.connect("http://localhost:8080");
    EXPECT_TRUE(result.is_ok());
    EXPECT_TRUE(adapter.is_connected());
}

TEST_F(WeaviateConnectionTest, ConnectWithHttpsUri) {
    auto result = adapter.connect("https://my-cluster.weaviate.network");
    EXPECT_TRUE(result.is_ok());
    EXPECT_TRUE(adapter.is_connected());
}

TEST_F(WeaviateConnectionTest, ConnectWithEmptyStringReturnsError) {
...
TEST_F(WeaviateConnectionTest, ConnectWithInvalidSchemeReturnsError) {
...
TEST_F(WeaviateConnectionTest, ConnectWithPostgresSchemeReturnsError) {
    auto result = adapter.connect("postgresql://localhost:5432/mydb");
    EXPECT_TRUE(result.is_err());
    EXPECT_EQ(result.error_code, ErrorCode::INVALID_ARGUMENT);
}

TEST_F(WeaviateConnectionTest, Disconnect) {
...
TEST_F(WeaviateConnectionTest, ConnectWithPortInUri) {
    auto result = adapter.connect("http://localhost:8080");
    EXPECT_TRUE(result.is_ok());
    EXPECT_TRUE(adapter.is_connected());
}
...
class WeaviateVectorTest : public ::testing::Test {
protected:
    void SetUp() override {
        adapter.connect("http://localhost:8080");
    }
...
TEST_F(WeaviateVectorTest, InsertAndSearchVectors) {
    Vector v1, v2, v3;
...
TEST_F(WeaviateVectorTest, SearchReturnsSortedByDescendingSimilarity) {
    Vector v1, v2;
    v1.data = {1.0f, 0.0f};
    v2.data = {0.0f, 1.0f};
...
TEST_F(WeaviateVectorTest, InsertVectorWithMetadata) {
    Vector v;
    v.data = {0.5f, 0.5f};
    v.metadata["class"] = Scalar{std::string{"Article"}};
...
TEST_F(WeaviateVectorTest, SearchWithMetadataFilter) {
    Vector v1, v2;
    v1.data = {1.0f, 0.0f};
    v1.metadata["class"] = Scalar{std::string{"Article"}};
    v2.data = {1.0f, 0.0f};
    v2.metadata["class"] = Scalar{std::string{"Question"}};
    ...
    filter["class"] = Scalar{std::string{"Article"}};
...
TEST_F(WeaviateVectorTest, InsertEmptyVectorReturnsError) {
    Vector empty;
...
TEST_F(WeaviateVectorTest, SearchEmptyVectorReturnsError) {
    Vector empty;
...
TEST_F(WeaviateVectorTest, SearchEmptyCollectionReturnsEmpty) {
    Vector query;
    query.data = {1.0f, 0.0f};

    auto result = adapter.search_vectors("NonExistentClass", query, 5);
    ASSERT_TRUE(result.is_ok());
    EXPECT_TRUE(result.value.value().empty());
}

TEST_F(WeaviateVectorTest, BatchInsertVectors) {
...
TEST_F(WeaviateVectorTest, CreateIndexSucceeds) {
    auto result = adapter.create_index(kCollection, 768);
    EXPECT_TRUE(result.is_ok());
}

TEST_F(WeaviateVectorTest, CreateIndexWithHnswParamsSucceeds) {
    ...
    params["vectorizer"]      = Scalar{std::string{"none"}};

    auto result = adapter.create_index(kCollection, 768, params);
    EXPECT_TRUE(result.is_ok());
}

TEST_F(WeaviateVectorTest, VectorIdGeneratedIsNotEmpty) {
    Vector v;
    ...
    ASSERT_TRUE(result.is_ok());
    EXPECT_FALSE(result.value.value().empty());
    EXPECT_NE(result.value.value().find("weaviate_obj_"), std::string::npos);
}

TEST_F(WeaviateVectorTest, MultipleInsertIdsAreUnique) {
    Vector v;
    v.data = {1.0f, 0.0f};
...
TEST_F(WeaviateVectorTest, OperationsWhileDisconnectedReturnError) {
    adapter.disconnect();
...
// ---------------------------------------------------------------------------
// Document adapter tests (Weaviate object store)
// ---------------------------------------------------------------------------

class WeaviateDocumentTest : public ::testing::Test {
protected:
    void SetUp() override {
        adapter.connect("http://localhost:8080");
    }
...
TEST_F(WeaviateDocumentTest, InsertAndRetrieveDocument) {
...
TEST_F(WeaviateDocumentTest, InsertGeneratesIdWhenEmpty) {
    Document doc;
...
TEST_F(WeaviateDocumentTest, InsertDuplicateIdReturnsError) {
...
TEST_F(WeaviateDocumentTest, BatchInsertDocuments) {
...
TEST_F(WeaviateDocumentTest, FindWithFilter) {
...
TEST_F(WeaviateDocumentTest, FindByIdFilter) {
...
TEST_F(WeaviateDocumentTest, FindNonExistentCollectionReturnsEmpty) {
    auto result = adapter.find_documents("NonExistentClass", {});
    ASSERT_TRUE(result.is_ok());
    EXPECT_TRUE(result.value.value().empty());
}

TEST_F(WeaviateDocumentTest, FindRespectsLimit) {
    for (int i = 0; i < 10; ++i) {
...
TEST_F(WeaviateDocumentTest, UpdateMatchingDocuments) {
...
TEST_F(WeaviateDocumentTest, UpdateWithEmptyUpdatesReturnsError) {
...
TEST_F(WeaviateDocumentTest, UpdateNonExistentCollectionReturnsZero) {
    std::map<std::string, Scalar> updates;
    updates["field"] = Scalar{std::string{"value"}};

    auto result = adapter.update_documents("NoSuchClass", {}, updates);
    ASSERT_TRUE(result.is_ok());
    EXPECT_EQ(result.value.value(), 0u);
}

TEST_F(WeaviateDocumentTest, OperationsWhileDisconnectedReturnError) {
    adapter.disconnect();
...
class WeaviateGraphTest : public ::testing::Test {
protected:
    void SetUp() override {
        adapter.connect("http://localhost:8080");
    }
...
TEST_F(WeaviateGraphTest, InsertNodeReturnsNotImplemented) {
    GraphNode node;
    node.id    = "node1";
    node.label = "Entity";
...
TEST_F(WeaviateGraphTest, InsertEdgeReturnsNotImplemented) {
    GraphEdge edge;
    edge.id        = "e1";
    edge.source_id = "n1";
    edge.target_id = "n2";
...
TEST_F(WeaviateGraphTest, ShortestPathReturnsNotImplemented) {
    auto result = adapter.shortest_path("n1", "n2", 5);
    EXPECT_TRUE(result.is_err());
    EXPECT_EQ(result.error_code, ErrorCode::NOT_IMPLEMENTED);
}

TEST_F(WeaviateGraphTest, TraverseReturnsNotImplemented) {
    auto result = adapter.traverse("n1", 3);
    EXPECT_TRUE(result.is_err());
    EXPECT_EQ(result.error_code, ErrorCode::NOT_IMPLEMENTED);
}

TEST_F(WeaviateGraphTest, ExecuteGraphQueryReturnsNotImplemented) {
...
class WeaviateRelationalTest : public ::testing::Test {
protected:
    void SetUp() override {
        adapter.connect("http://localhost:8080");
    }
...
TEST_F(WeaviateRelationalTest, ExecuteQueryReturnsNotImplemented) {
    auto result = adapter.execute_query("SELECT * FROM table");
    EXPECT_TRUE(result.is_err());
    EXPECT_EQ(result.error_code, ErrorCode::NOT_IMPLEMENTED);
}

TEST_F(WeaviateRelationalTest, InsertRowReturnsNotImplemented) {
    RelationalRow row;
...
TEST_F(WeaviateRelationalTest, BatchInsertReturnsNotImplemented) {
    RelationalRow row;
    row.columns["col"] = Scalar{std::string{"val"}};
...
TEST_F(WeaviateRelationalTest, GetQueryStatisticsReturnsOk) {
...
class WeaviateTransactionTest : public ::testing::Test {
protected:
    void SetUp() override {
        adapter.connect("http://localhost:8080");
    }
...
TEST_F(WeaviateTransactionTest, BeginTransactionReturnsNotImplemented) {
    auto result = adapter.begin_transaction();
    EXPECT_TRUE(result.is_err());
    EXPECT_EQ(result.error_code, ErrorCode::NOT_IMPLEMENTED);
}

TEST_F(WeaviateTransactionTest, CommitTransactionReturnsNotImplemented) {
    auto result = adapter.commit_transaction("txn_001");
    EXPECT_TRUE(result.is_err());
    EXPECT_EQ(result.error_code, ErrorCode::NOT_IMPLEMENTED);
}

TEST_F(WeaviateTransactionTest, RollbackTransactionReturnsNotImplemented) {
...
class WeaviateSystemInfoTest : public ::testing::Test {
protected:
    void SetUp() override {
        adapter.connect("http://localhost:8080");
    }
...
TEST_F(WeaviateSystemInfoTest, GetSystemInfoReturnsWeaviate) {
    auto result = adapter.get_system_info();
    ASSERT_TRUE(result.is_ok());
    EXPECT_EQ(result.value.value().system_name, "Weaviate");
    EXPECT_FALSE(result.value.value().version.empty());
}

TEST_F(WeaviateSystemInfoTest, GetMetricsReturnsOk) {
    auto result = adapter.get_metrics();
    EXPECT_TRUE(result.is_ok());
}

TEST_F(WeaviateSystemInfoTest, HasVectorSearchCapability) {
    EXPECT_TRUE(adapter.has_capability(Capability::VECTOR_SEARCH));
}

TEST_F(WeaviateSystemInfoTest, HasDocumentStoreCapability) {
    EXPECT_TRUE(adapter.has_capability(Capability::DOCUMENT_STORE));
}

TEST_F(WeaviateSystemInfoTest, HasFullTextSearchCapability) {
    EXPECT_TRUE(adapter.has_capability(Capability::FULL_TEXT_SEARCH));
}

TEST_F(WeaviateSystemInfoTest, HasBatchOperationsCapability) {
    EXPECT_TRUE(adapter.has_capability(Capability::BATCH_OPERATIONS));
}

TEST_F(WeaviateSystemInfoTest, HasSecondaryIndexesCapability) {
    EXPECT_TRUE(adapter.has_capability(Capability::SECONDARY_INDEXES));
}

TEST_F(WeaviateSystemInfoTest, DoesNotHaveRelationalCapability) {
    EXPECT_FALSE(adapter.has_capability(Capability::RELATIONAL_QUERIES));
}

TEST_F(WeaviateSystemInfoTest, DoesNotHaveGraphCapability) {
    EXPECT_FALSE(adapter.has_capability(Capability::GRAPH_TRAVERSAL));
}

TEST_F(WeaviateSystemInfoTest, DoesNotHaveTransactionCapability) {
    EXPECT_FALSE(adapter.has_capability(Capability::TRANSACTIONS));
}

TEST_F(WeaviateSystemInfoTest, GetCapabilitiesReturnsExpectedSet) {
...
// ---------------------------------------------------------------------------
// Adapter factory registration test
// ---------------------------------------------------------------------------

class WeaviateFactoryTest : public ::testing::Test {
protected:
    void SetUp() override {
        // Register once per test suite. AdapterFactory::register_adapter() returns
        // false (without overwriting the existing entry) if the system name is
        // already registered, so calling this multiple times is safe.
        AdapterFactory::register_adapter("Weaviate",
            []() { return std::make_unique<WeaviateAdapter>(); });
    }

    static constexpr const char* kConnString = "http://localhost:8080";
};

TEST_F(WeaviateFactoryTest, RegisterAndCreate) {
    EXPECT_TRUE(AdapterFactory::is_supported("Weaviate"));

    auto adapter = AdapterFactory::create("Weaviate");
    ASSERT_NE(adapter, nullptr);
    EXPECT_FALSE(adapter->is_connected());
}

TEST_F(WeaviateFactoryTest, ConnectViaFactory) {
    auto adapter = AdapterFactory::create("Weaviate");
    ASSERT_NE(adapter, nullptr);
...
TEST_F(WeaviateFactoryTest, CapabilitiesViaFactory) {
    auto adapter = AdapterFactory::create("Weaviate");
    ASSERT_NE(adapter, nullptr);
    EXPECT_TRUE(adapter->has_capability(Capability::VECTOR_SEARCH));
    EXPECT_TRUE(adapter->has_capability(Capability::DOCUMENT_STORE));
...
class WeaviateSecurityTest : public ::testing::Test {
protected:
    WeaviateAdapter adapter;
};

TEST_F(WeaviateSecurityTest, ApiKeyNotExposedInSystemInfo) {
    // Connect with an API key passed via the options map.
    adapter.connect("https://my-cluster.weaviate.network",
...
TEST_F(WeaviateSecurityTest, ConnectWithoutApiKeySucceeds) {
...
TEST_F(WeaviateSecurityTest, ConnectWithApiKeyInOptionsSucceeds) {
    auto result = adapter.connect("https://my-cluster.weaviate.network",
...
TEST_F(WeaviateSecurityTest, SystemInfoEndpointDoesNotLeakKey) {
    adapter.connect("https://cluster.weaviate.network",
                    {{"api_key", "super-secret-weaviate-key"}});
...
//
// All tests run against the in-process simulation mode (no live Weaviate
...
class WeaviatePerformanceTest : public ::testing::Test {
protected:
    void SetUp() override {
        adapter.connect("http://localhost:8080");
    }
...
TEST_F(WeaviatePerformanceTest, BulkDocumentInsertOverhead) {
    std::vector<Document> docs;
...
TEST_F(WeaviatePerformanceTest, BulkVectorInsertOverhead) {
...
TEST_F(WeaviatePerformanceTest, VectorSearchOverhead) {
    for (size_t i = 0; i < kVecCount; ++i) {
        Vector v;
        v.data.resize(kVecDim, static_cast<float>(i) / kVecCount);
